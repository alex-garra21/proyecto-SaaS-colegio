...
export default function Encargados() {
  ...
  // Estados del modal de agregar encargado
  ...
  const fetchEncargados = async () => {
    ...
    try {
      ...
      if (!res.ok) {
        throw new Error('Fallo al obtener los encargados de la base de datos.');
      }

      const data = await res.json() as UserData[];
      const encargadosOnly = data.filter(u => u.IdRol === 5);
      setUsersList(encargadosOnly);
    } catch (err: any) {
      console.error(err);
  ...
  // Registrar encargado manual
  const handleAddEncargadoSubmit = async (e: React.FormEvent) => {
    e.preventDefault();
    ...
    try {
      ...
      const res = await fetch(`${apiBaseUrl}/api/users`, {
        ...
        body: JSON.stringify({
          ...
          idRol: 5 // Padre / Encargado por defecto
        })
      });

      if (!res.ok) {
        const errData = await res.json();
        throw new Error(errData?.message || 'Fallo al registrar el encargado.');
      }
      ...
      showToast(`¡Encargado ${fullname} registrado exitosamente!`, 'success');
  ...
  const sortedEncargados = useMemo(() => {
    const sorted = [...filteredEncargados];
    sorted.sort((a, b) => {
    ...
    return sorted;
  }, [filteredEncargados, sortField, sortOrder]);

  const paginatedEncargados = useMemo(() => {
    const startIndex = (currentPage - 1) * itemsPerPage;
    return sortedEncargados.slice(startIndex, startIndex + itemsPerPage);
  }, [sortedEncargados, currentPage]);

  const totalPages = Math.max(1, Math.ceil(filteredEncargados.length / itemsPerPage));
      ...
      <div className="flex flex-col sm:flex-row items-start sm:items-center justify-between gap-4 border-b border-slate-100 pb-4">
        <div>
          ...
          <h1 className="text-2xl font-extrabold tracking-tight text-slate-900 mt-1">
            Gestión de Padres / Encargados
          </h1>
          <p className="text-xs text-[#64748B] mt-0.5">
            Consulta de padres de familia y vinculaciones familiares en el portal de solvencia.
          </p>
        </div>
        ...
        >
          <UserPlus size={15} />
          <span>Agregar Encargado Manual</span>
        </button>
      </div>
        ...
        {filteredEncargados.length > 0 && (
          <div className="px-5 py-3 border-t border-slate-100 flex items-center justify-between bg-slate-50/30">
            <span className="text-[10px] text-slate-500 font-semibold">
              Mostrando del {Math.min(filteredEncargados.length, (currentPage - 1) * itemsPerPage + 1)} al {Math.min(filteredEncargados.length, currentPage * itemsPerPage)} de {filteredEncargados.length} encargados
            </span>
      ...
      {isModalOpen && (
        <div className="fixed inset-0 flex items-center justify-center p-4 bg-slate-900/40 backdrop-blur-sm z-50 animate-in fade-in duration-200">
          ...
          <div className="relative w-full max-w-md bg-white rounded-lg border border-[#E2E8F0] shadow-2xl p-6 space-y-4 animate-in zoom-in-95 duration-200 z-10 text-left">
            ...
            <form onSubmit={handleAddEncargadoSubmit} className="space-y-4">
              ...
              <div className="flex justify-end gap-3 pt-3 border-t border-slate-100">
                ...
                >
                  {modalLoading ? (
                    ...
                    <span>Registrar e Inyectar Encargado</span>
                  )}
                </button>
              </div>
            </form>
